    			...
    			<article class="<?php echo esc_attr( join( ' ', get_post_class( array( 'course', 'post' ), $post_id ) ) ); ?>">
    				...
    				<section class="entry">
    					<p class="sensei-course-meta">
    					   ...
    					   <span class="course-author"><?php _e( 'Taught by ', 'woothemes-sensei' ); ?><a href="<?php echo $author_link; ?>" title="<?php echo esc_attr( $author_display_name ); ?>"><?php echo esc_html( $author_display_name   ); ?></a>.</span>
    					   <?php } // End If Statement ?>
    					   <span class="course-lesson-count"><?php echo 'This course contains ' . $woothemes_sensei->post_types->course->course_lesson_count( $post_id ) . apply_filters( 'sensei_lessons_text', __( ' lessons.', 'woothemes-sensei' ) ); ?></span>
                           <?php if ( '' != $category_output ) { ?>
                           		<span class="course-category"><?php echo sprintf( __( 'It is categorized as a %s.', 'woothemes-sensei' ), $category_output ); ?></span>
                           <?php } // End If Statement ?>
    					   <?php sensei_simple_course_price( $post_id ); ?>
                        </p>
                        <p class="course-excerpt"><?php echo $post_item->post_content	; ?></p>
    				</section>
    			</article>
    			<?php
		...
		<section id="main-course" class="course-container">
			...
			<article class="<?php echo esc_attr( join( ' ', get_post_class( array( 'course', 'post' ), get_the_ID() ) ) ); ?>">
    			...
    			<section class="entry">
                    ...
                    <p class="course-excerpt"><?php echo apply_filters( 'get_the_excerpt', $post_item->post_content ); ?></p>
    			</section>
    		</article>
